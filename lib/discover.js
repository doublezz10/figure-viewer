...
function discoverFiguresInSubdirs(startPath, config = null, depth = 0, maxDepth = 10) {
  const cfg = config || getConfig();
  const figureDirs = cfg.figuresDirs || DEFAULT_CONFIG.figuresDirs;
  ...
  if (!fs.existsSync(searchDir) || depth > maxDepth) return found;
  ...
  for (const entry of entries) {
    ...
    for (const figureDir of figureDirs) {
      ...
      if (fs.existsSync(candidatePath) && fs.statSync(candidatePath).isDirectory()) {
        ...
        const relPath = path.relative(searchDir, candidatePath);
        found.push({
          dir: candidatePath,
          parent: relPath.replace(`/${figureDir}`, ''),  // e.g., "analysis_type/specific_analysis"
          figureDir: figureDir
        });
      }
    }
    ...
    const subResults = discoverFiguresInSubdirs(
